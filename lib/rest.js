...
function handleGet(req, res, next) {
  debug(`GET-request recieved. db:${req.params.db} collection:${req.params.collection} id:${req.params.id || "*"}`);
  var query;
...
server.put('/:db/:collection/:id', function (req, res) {
  debug(`PUT-request recieved. db:${req.params.db} collection:${req.params.collection} id:${req.params.id}`);
  var spec = {
    '_id': new ObjectID(req.params.id)
  };
  MongoClient.connect(util.connectionURL(req.params.db, config), function (err, db) {
